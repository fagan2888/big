...
#Expression = ['BIAS_5#-10#0&thre','BIAS_5#10#1&thre','BIAS_5#BIAS_50&cross*BIAS_5#0#1&thre','BIAS_5#BIAS_50&cross*BIAS_5#0#0&thre','RSI_6#RSI_12&cross']
#Expression = ['MACD_12#MACD_26&cross*WR#80#1&thre','MACD_12#MACD_26&cross+WR#20#0&thre']
#Expression = ['MACD_12#MACD_26&cross','MACD_12#MACD_26&cross*CCI#-100#1&thre*CCI#100#0&thre']
#Expression = ['MACD_12#MACD_26&cross']#'MACD_12#MACD_26&cross*RSI_6#20#0&thre']
#Expression = ['close_EMA_5#close_EMA_10&cross*MACD#0#1&thre','J#D&cross*K#D&cross*J#70#0&thre','RSI_6#RSI_12&cross*RSI_6#80#0&thre','close_EMA_12#close_EMA_26&cross','RSI_6#85#1&thre','J#100#1&thre','MACD#0#0&thre&HS']
#Expression = ['close_EMA_5#close_EMA_10&cross','J#D&cross*K#D&cross*J#70#0&thre','RSI_6#RSI_12&cross*RSI_6#80#0&thre','close_EMA_12#close_EMA_26&cross','RSI_6#85#1&thre','J#100#1&thre','MACD#0#0&thre&HS']
#Expression = ['close_EMA_5#close_EMA_10&cross','J#D&cross*K#D&cross*J#70#0&thre','RSI_6#RSI_12&cross*RSI_6#80#0&thre','close_EMA_12#close_EMA_26&cross','RSI_6#RSI_12&cross','J#D&cross*K#D&cross','MACD#0#0&thre&HS']
#Expression = ['close_EMA_5#close_EMA_10&cross','J#D&cross*K#D&cross*J#70#0&thre','RSI_6#RSI_12&cross*RSI_6#80#0&thre','close_EMA_12#close_EMA_26&cross','RSI_6#RSI_12&cross','J#D&cross*K#D&cross','MACD#0#0&thre&HS']
Expression = ['close_EMA_5#close_EMA_10&cross','close_EMA_12#close_EMA_26&cross','RSI_6#85#1&thre','J#100#1&thre','close_EMA_12#close_EMA_26&cross&HS','K#40#1&thre&HS']

#dp_code = '999999.XSHG'
dp_code = 'BABA'
dp_trade_date = sig_data_us.get_dp_trade_date(dp_code)#上证指数的交易日期列表
...
class Worth:
    ...
    def __init__(self,code = '',w = 10,gold = 0,position = 0,asset = 0,buy_price = 0,sell_price = 0,now_date = '',
                pure_value = 1,date = '',operation = 0,original_gold = Original_gold,buy_due = 0,sell_due = 0,trade_date_list = [],
                now_bar = [],obuy_price = 0,limit_move = [0,0],expre_Sig = []):
        self.code = code #股票代码 
        ...
        self.limit_move = limit_move #当天是否涨停，第一个位置为1为涨停，第二个位置为1为跌停
        self.expre_Sig =  expre_Sig
    #某一天的买入卖出函数
    ...
    def one_code_trade(self,):
        ...
        #all_buy_price = 100*self.buy_price#加入了滑点和手续费
        #all_sell_price = 100*self.sell_price#加入了滑点和手续费
        ...
        is_make_money = 0
        #买入
        #print('buy',self.operation>0,self.gold>all_buy_price,self.buy_price!=0)
        if(self.operation>0 and self.gold>all_buy_price and self.buy_price!=0): 
            if(self.buy_price<1.1*settle):#涨停买不进
                ...
                self.limit_move[0] = 1
        #卖出
        elif(self.operation<0 and self.position>0 and self.sell_price!=0):
            if(self.sell_price>0.9*settle and self.sell_price<1.1*settle):#卖出价格为0会出现资产断崖式下跌
                trade_gold = all_sell_price*self.position            #跌停卖不出，涨停不卖
    ...
    def get_operation(self,):
        if(not self.now_bar.empty):
            ...
            if(self.expre_Sig[0] == 2 and self.expre_Sig[5] == 1): #or self.expre_Sig[2] == 2):
                self.operation = 1
    ...
    def get_trade_price(self,):
        if(not self.now_bar.empty and 'close' in self.now_bar.keys()):
            self.buy_price = self.now_bar['open'].iloc[-1] #以当天开盘价为买入价格
    ...
    def get_now_bar(self,now_date):
        ...
        data = sig_data_us.get_windows_data(self.code,self.now_date,self.w)
    ...
    def update_Worth(self,):
        ...
        if(not self.now_bar.empty and 'close' in self.now_bar.keys()):
            return self.now_bar['close'].iloc[-1],sig_data_us.get_windows_data(dp_code,self.now_date,self.w)['close'].iloc[-1]
        else:
            return 0,0
class Result:
...
<<<<<<< HEAD:src/sig_backtest_ls_IF.py
        self.Sig = sig_fra.Signal(expression= Expression,code_list = self.ccsl,date = _nowdate,HScode = 'IF888')#创建信号系统
=======
        self.Sig = sig_fra.Signal(expression= Expression,code_list = self.ccsl,date = _nowdate,HScode='BABA')#创建信号系统
>>>>>>> parent of 4c7cf4b... IF:src/sig_backtest_with_choose_stock_us.py
    ...
    def get_tbsl(self,):
        ...
        for _code in self.ccsl:
            #print('Sig',self.Sig.expre_sig)
            _worth = Worth(code = _code,gold = 0,position = 0,asset = 0,buy_price = 0,w = 100,
                          sell_price = 0,pure_value = 1,date = self._nowdate,operation = 0,obuy_price = 0,
                          expre_Sig=copy.copy(self.Sig.expre_sig[_code]))#买入用的是昨天的信号
            ...
            #把今天要买的股票加入列表
            if(_worth.date in _worth.trade_date_list and (_worth.get_operation() == 1) and _worth.code not in self.hbsl):
                self.tbsl.append(_code)    
                self.tbsW.append(_worth)
            #print('result',_worth.now_date,self.Sig.expre_sig,self.Sig.signal,self.Sig.life)
    ...
    def get_tssl(self,):
        ...
        for _code,_worth in zip(self.hbsl,self.hbsW):
            ...
            if(_worth.now_date in _worth.trade_date_list and (_worth.sell_due <= 0#):
                and (_worth.get_operation() == -1) or _code not in self.ccsl)): #出现卖的信号或者换仓的时候平仓
    ...
    def after_close(self,):
        ...
        #print(self.Sig.signal)
        #print(self.Sig.type_list)
        self.Sig.update()
    ...
    def trade(self,):
        ...
        #print(self._nowdate)
        self.get_asset()
        print(self._nowdate,self.asset)
...
def get_code_list():
    ...
    for i in range(len(code_list)):
        if(str(code_list[i])[0]=='6'):
            code_list[i] = str(code_list[i]) + '.SH'
        else:
            code_list[i] = str(code_list[i]) + '.SZ'
...
def All_trade(code_list,begin_date):
    para_name = 'year/'
    para_data,trade_data = {},{}
    #begin_date = int(dp_trade_date[0])#20180103
    #try:
    ori_gold = 100000000
    C_S = CStock(ori_gold = ori_gold,_nowdate = begin_date,could_choose_stocks_list = _code_list,all_choose_stocks_list = _code_list,
                    ...
                    gold_list = [],code_list = [],is_make_money = [],expre_sig_list =[]),today_sell_stocks_list = [])
    ...
    '''
        ...
        print(e)
        para_data,trade_data = para_result(para_data,trade_data,_para,C_S)
        para_data = pd.DataFrame(para_data,index = C_S.result.date_list)
        para_data.to_excel(result_save_path+para_name+"_pure.xlsx", header=True, index=True)
        trade_data = pd.DataFrame(trade_data,index = C_S.result.trade_date_list)
        trade_data.to_excel(result_save_path+para_name+"_trade.xlsx", header=True, index=True)
    '''
...
if __name__ == "__main__":
    #code_list = ['510050.XSHG','510500.XSHG','510300.XSHG']
    ...
    if(not off_line):
        for code in code_list:
            if(sig_data_us.cal_index_data(code) == 0):
                _code_list.remove(code)
    #########
        #sig_data_us.cal_index_data('999999.XSHG')
    #if('999999.XSHG' in code_list):
        #_code_list.remove('999999.XSHG')
    #########
    begin_year = 2016  
    begin_date = dp_trade_date[dp_trade_date>=(str(begin_year)+'0101')][0]
    #WD_pv,WD_sharpe,WD_max_draw_down,hold_pv,hold_sharpe,hold_max_draw_down,win_rate = All_trade(code_list,begin_date,off_line=False)
    WD_pv = All_trade(_code_list,begin_date)
    print(WD_pv)