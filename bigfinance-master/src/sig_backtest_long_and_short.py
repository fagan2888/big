...
#Expression = ['close#close_EMA_5&cross','close_EMA_5#2#1&trend','close_EMA_5#2#0&trend']
Expression = ['close_EMA_12#close_EMA_26&cross','RSI_6#RSI_12&cross','J#D&cross*K#D&cross']
dp_code = '999999.XSHG'
    ...
    def get_operation(self,):
        if(not self.now_bar.empty):
            if(self.expre_Sig[0] == 2 and self.expre_Sig[1] == 2 and self.expre_Sig[2] == 4): #or self.expre_Sig[2] == 2):
                ...
                self.operation = 'sell'
            print(self.expre_Sig[0],self.expre_Sig[1],self.expre_Sig[2])
    ...
    def get_tbsl(self,):
        ...
        for _code in self.ccsl:
            ...
            if(_worth.date in _worth.trade_date_list and 
                (_worth.get_operation() == 'long' or _worth.get_operation() == 'short') 
...
def All_trade(code_list,begin_date,result_save_path = result_save_path,Expression = Expression):
    ...
    C_S = CStock(ori_gold = ori_gold,_nowdate = begin_date,could_choose_stocks_list = code_list,all_choose_stocks_list = code_list,
                    ...
                    gold_list = [],code_list = [],is_make_money = [],expre_sig_list =[]),today_sell_stocks_list = [],Expression = Expression)
...
if __name__ == "__main__":
    main(result_save_path = result_save_path,Expression = Expression)