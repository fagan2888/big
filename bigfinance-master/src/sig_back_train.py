...
op_path = '/root/yingke/result/'

buy_fee = 1.0003 #买入手续费万3
sell_fee = 0.9997 #卖出手续费万3
Slippage = 0.003 #滑点
buy_need = 1.03 #上界
sell_need = 0.98 #下界
#Slippage = 0
...
class Worth:
    ...
    def __init__(self,code = '',w = 10,gold = 0,position = 0,asset = 0,buy_price = 0,sell_price = 0,now_date = '',
                pure_value = 1,date = '',operation = 0,original_gold = Original_gold,buy_due = 1,sell_due = 1,trade_date_list = [],
    ...
    def one_code_trade(self,):
        ...
        is_make_money = 0
        #买入
        #print(self.sell_price,self.buy_price,settle,self.sell_price>0.9*settle,self.buy_price<1.1*settle)
        if(self.sell_price>0.9*settle and self.buy_price<1.1*settle):#涨跌停不能买卖
            if(self.operation>0 and self.gold>all_buy_price and self.buy_price!=0):
                #print('buy',self.now_date,self.code)
	        ...
	        #卖出
                ...
                #print('sell',self.now_date,self.code)
        ...
        return self.position,trade_gold,close,trade_price,self.now_date,is_make_money
    
    #得到操作信号
    def get_operation(self,):
        if(os.path.exists(op_path+self.code+'_one_pre_result.xlsx')):
            ope = pd.read_excel(op_path+self.code+'_one_pre_result.xlsx',index_col = 0)
            if(int(self.now_date) in ope.index.tolist()):
                #print(self.now_date,ope.loc[self.now_date])
                self.operation = int(ope.loc[self.now_date])
...
class CStock:#计算选股模型的净值
    ...
    def buy_bar(self,):
        ...
        for _code,_worth in zip(self.tbsl,self.tbsW):#####这里以后可能还要改，因为可能有些信号不是立即买，那么分配钱就有问题#######
            ...
            if(_worth.buy_due <= 0):
                #print(self._nowdate,_worth.buy_due)
                position,trade_gold,close,trade_price,trade_date,is_make_money = _worth.one_code_trade()
                if(trade_gold>0): #如果交易金额大于0
                    self.add_result(_code,position,trade_gold,close,trade_price,trade_date,is_make_money)
    ...
    def get_tssl(self,):
        ...
        for _code,_worth in zip(self.hbsl,self.hbsW):
            ...
            #print(_worth.now_date in _worth.trade_date_list,_worth.sell_due <= 0,_code not in self.ccsl)
    ...
    def add_result(self,code,position,trade_gold,close,trade_price,trade_date,is_make_money):
...
def All_trade(code_list,begin_date):
    ...
    C_S = CStock(ori_gold = ori_gold,_nowdate = begin_date,could_choose_stocks_list = _code_list,all_choose_stocks_list = _code_list,
                    ...
                    gold_list = [],code_list = [],is_make_money = []),today_sell_stocks_list = [])
...
if __name__ == "__main__":
    #code_list = ['002537.XSHE','600000.XSHG']
    ...
    if(not off_line):
        for code in code_list:
            if((code[0] != '6' and code[0] != '0') or sig_data.cal_index_data(code) == 0):
